        ...
        function createTextWithImage(paragraph){
            ...
            paragraph.forEach(item =>{
                ...
                if (item.custom){
                    ...
                    textElement.classList.add('custom_iframe');
                    textElement.innerHTML = item.custom; // Использование innerHTML для поддержки HTML-тегов
                    container.appendChild(textElement);
                };
                if (item.text2){
                    const textElement = document.createElement('div');
                ...
                if (item.image){
                    ...
                    if(item.no_sgn){
                        imgContainer.className = 'img_with_sgn no_sgn';
                    }
                    let imgElement;
            ...
            if (pageData.hasOwnProperty('paragraph_1')){
                ...
                for (let key in pageData){
                    if (key.startsWith('paragraph_')){
                        ...
                        } else if (paragraph.some(item => item.text) || paragraph.some(item => item.custom) || paragraph.some(item => item.text2)){
                ...
                let iframe = document.querySelector('#customFrame') || null;
                
                if (iframe != null) {
                    iframe.onload = function() {   // При загрузке контента
                        iframe.style.height = iframe.contentWindow.document.body.scrollHeight + 50 + 'px'; // Подгоним высоту под содержимое
                    };
                    window.addEventListener('resize', () => {
                        let iframe = document.querySelector('#customFrame') || null;
                        if(iframe != null) {
                            // Сбросим высоту перед вычислением для корректного уменьшения
                            iframe.style.height = 'auto'; 
                            // Теперь обновим высоту на основе актуального содержимого
                    ...
                    if (testItem.hasOwnProperty('type')){
                        switch (testItem.type){
                            ...
                            case 9:
                                ...
                                break;
                            case 10:
                                replaceScript('./scripts/script_of_tests/test_type_9.js', 'test-script');
                                break;
                            default:
